...
public class BookingController {
    ...
    public float getSuggestionHours() {
        ...
        for (int i = 0; i < getAddons().size(); i++){
            if(getAddons().get(i).getInt(AddonServiceAPIParams.JOB_ADDONS_ID) == AddonId.WINDOWS){
                suggestionTime += 0.5f;
            }

            if(getAddons().get(i).getInt(AddonServiceAPIParams.JOB_ADDONS_ID) == AddonId.LAUNDRY){
                suggestionTime += 1f;
            }
        }
        ...
        if(isHouseKeeper() && suggestionTime < 3){
            return 3;
        }
    ...
    public boolean isMasseur() {
        return job.getTypeJob().getEnumValue() == TypeJobEnum.MASSEUR;
    }

    public boolean isGoGoBoy() {
        return job.getTypeJob().getEnumValue() == TypeJobEnum.GOGO_BOY;
    }

    public boolean isCarWash() {
        return job.getTypeJob().getEnumValue() == TypeJobEnum.CAR_WASH;
    }

    public boolean isPoolBoy() {
        return job.getTypeJob().getEnumValue() == TypeJobEnum.POOL_BOY;
    }
